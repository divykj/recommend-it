      ...
      <svg>
        ...
        <circle
          ...
          style={{
            ...
            stroke: `${
              number > 70 ? "#00ff43" : number > 30 ? "fdcc0d" : "#DE0913"
            }`,
          }}
        ></circle>
      </svg>
  ...
  useEffect(() => {
    const getMoviePoster = async () => {
      ...
      const {
        ...
        `http://www.omdbapi.com/?i=tt3896198&apikey=3f3336a&t=${name}&y=${year}`
      );
        ...
        {loading ? (
          ...
          <>
            <div
              ...
              style={{
                backgroundImage:
                  !loading && moviePoster !== "" && `url(${moviePoster})`,
              }}
            >
              {!moviePoster && (
                <>
...
const App = () => {
  ...
  const addtoFavorites = (val) => {
    setFavourites((oldFavorites) =>
      oldFavorites.findIndex(({ id }) => id === val.id) === -1
        ? [...oldFavorites, val]
        : oldFavorites
    );
  };
    ...
    <GeistProvider theme={myTheme}>
      ...
      <Page>
        <Row justify="center" className="navbar">
          <img src="/logo.png" alt="" width={60} height={60} /> <p>Recommend</p>
          <Text type="warning">It</Text>
        </Row>
        <SearchForm onSelected={addtoFavorites} />
        <Card className="mt">
          <h3>Favourites List</h3>
          {favourites.length !== 0 ? (
            favourites &&
            favourites.map(({ id, label }) => (
              <Tag
                key={id}
                type="dark"
                className="mr tag"
            ...
            <Text blockquote>
              Add your favourite movies and get amazing recommendations for your
              likings!
            </Text>
          )}
          {favourites.length > 0 && (
        ...
        {recommendationData ? (
          recommendationData.length === 0 ? (
            ...
            recommendationData &&
            recommendationData.map(({ recommendations, movie }) => (
              <div key={movie.name}>
